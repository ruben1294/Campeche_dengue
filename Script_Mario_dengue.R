# ==============================================================
# ANÁLISIS DENGUE–CLIMA EN CAMPECHE · PIPELINE REPRODUCIBLE
# ==============================================================
# Ejecuta:  Rscript Script_Mario_dengue.R
# Un solo comando descarga insumos, procesa, modela y genera
# figuras en PNG 300 DPI con estilo científico minimalista.
#
# Fuentes (todas públicas, descarga automatizada):
...
download_cached <- function(url, dest, label) {
  ...
  tryCatch(
    ...
    error = function(e) {
      stop(sprintf("Falló la descarga de %s: %s\nVerifica tu conexión.",
...
parse_oni <- function(path) {
  ...
  tab <- read.table(text = data_rows, header = FALSE,
...
process_data <- function(power, oni, dengue) {
  power_daily <- power |>
    mutate(date  = as.Date(doy - 1, origin = sprintf("%04d-01-01", year)),
  ...
  monthly <- power_daily |>
    group_by(year, mo) |>
    summarise(t_max  = mean(t_max, na.rm = TRUE),
  ...
  list(daily = power_daily, monthly = monthly,
...
run_stats <- function(df_annual) {
  ...
  spearman <- map_dfr(PREDICTORS, function(v) {
    ...
    tibble(variable = v, rho = unname(ct$estimate),
           p = ct$p.value, stars = sig_stars(ct$p.value))
  })

  # Matriz Spearman completa (con p)
  cor_mat <- cor(df_annual[, vars], method = "spearman",
...
make_figures <- function(data, stats) {
  ...
  climatology <- dm |>
    ...
    mutate(var = factor(var,
      ...
      labels = c("Temp. media (°C)", "Humedad relativa (%)",
                 "Precipitación (mm/mes)")))

  p1 <- ggplot(climatology, aes(mo, val)) +
    geom_col(data = ~ filter(., var == "Precipitación (mm/mes)"),
             fill = "grey75", colour = "grey30", linewidth = 0.3) +
    geom_line(data = ~ filter(., var != "Precipitación (mm/mes)"),
              colour = "grey25", linewidth = 0.6) +
    geom_point(data = ~ filter(., var != "Precipitación (mm/mes)"),
  ...
  clim_baseline <- dm |>
    group_by(mo) |>
    summarise(t_med_clim = mean(t_med),
              hr_clim    = mean(hr),
  ...
  p2 <- ggplot(anom_2023, aes(mo, anom, fill = anom > 0)) +
    ...
    scale_fill_manual(values = c(`TRUE` = "#b2182b", `FALSE` = "#2166ac"),
                      guide = "none") +
    labs(x = "Mes", y = "Anomalía vs media 2013–2024",
  ...
  df_long <- da |>
    ...
    mutate(variable = factor(variable, levels = vars_all,
  ...
  p4 <- df_long |>
    ...
    geom_histogram(bins = 6, fill = "grey85", colour = "grey20",
                   linewidth = 0.3) +
    geom_text(data = sh, aes(x = -Inf, y = Inf, label = label),
  ...
  cor_long <- as.data.frame(as.table(stats$cor_mat)) |>
    ...
    mutate(p     = as.vector(stats$p_mat),
           stars = sig_stars(p),
           label = ifelse(var1 == var2, "",
                          sprintf("%.2f\n%s", rho, stars)),
           var1  = factor(var1, levels = vars_all,
                          labels = unname(labels_all)),
  ...
  p5 <- ggplot(cor_long, aes(var1, var2, fill = rho)) +
    ...
    scale_fill_gradient2(low = "#2166ac", mid = "white", high = "#b2182b",
                         ...
                         name = expression(rho)) +
    coord_fixed() +
    labs(title = "Correlaciones de Spearman entre variables",
         x = NULL, y = NULL) +
    theme_sci() +
  ...
  xlims <- as.Date(c(sprintf("%d-01-01", yr_range[1]),
  ...
  p7_top <- ggplot(oni_sub, aes(date, oni)) +
    ...
    geom_hline(yintercept = c(-0.5, 0, 0.5), colour = "grey60",
               linewidth = 0.25, linetype = "dotted") +
    geom_line(colour = "grey25", linewidth = 0.5) +
    geom_point(colour = "black", size = 0.7) +
    scale_x_date(limits = xlims, date_breaks = "1 year",
                 date_labels = "%Y") +
    labs(x = NULL, y = "ONI (anomalía SST, °C)",
  ...
  fp <- stats$nb_summary |>
    mutate(variable = factor(variable, levels = rev(PREDICTORS),
  ...
  p8 <- ggplot(fp, aes(IRR, variable)) +
    ...
    scale_x_log10() +
    labs(x = "Razón de tasas (IRR, escala log)", y = NULL,
  ...
  p9 <- ggplot(da, aes(anio)) +
    ...
    labs(x = "Año", y = "Casos de dengue",
         title = sprintf("Casos observados vs predichos · NB ~ %s",
                         best_lab),
  ...
  g1 <- ggplot(diag_df, aes(fitted, rp)) +
    ...
    geom_point(size = 2) +
    labs(x = "Ajustados", y = "Residuos Pearson",
         title = "(a) Residuos vs ajustados") + theme_sci()
  g2 <- ggplot(diag_df, aes(sample = rd)) +
    stat_qq(size = 2) + stat_qq_line(linewidth = 0.4) +
    labs(x = "Cuantil teórico", y = "Residuos devianza",
         title = "(b) Q–Q de residuos") + theme_sci()
  g3 <- ggplot(diag_df, aes(anio, rp)) +
    geom_hline(yintercept = 0, colour = "grey60", linewidth = 0.3) +
    geom_line(colour = "grey40", linewidth = 0.4) +
    geom_point(size = 2) +
    scale_x_continuous(breaks = da$anio) +
    labs(x = "Año", y = "Residuos Pearson",
         title = "(c) Residuos en el tiempo") + theme_sci()
  ...
  bl_lab  <- sprintf("Box–Ljung (lag 3): χ² = %.2f, p = %.3f %s",
  ...
  g4 <- ggplot(acf_df, aes(lag, acf)) +
    ...
    geom_point(size = 1.6) +
    annotate("text", x = Inf, y = Inf, label = bl_lab,
             hjust = 1.05, vjust = 1.4, size = 3) +
    labs(x = "Rezago", y = "ACF",
         title = "(d) Autocorrelación de residuos") + theme_sci()
  ...
  conagua_ref <- tibble(
    ...
    precip_cn = c(1698.5,1390.1,1326.1,1177.0,1407.3,1389.8,
  ...
  comp <- da |>
    ...
    pivot_longer(-anio) |>
    mutate(fuente = ifelse(str_ends(name, "_cn"), "CONAGUA", "NASA POWER"),
           var    = str_remove(name, "_cn$"),
           var    = factor(var,
             levels = c("t_max","t_med","t_min","precip"),
             labels = c("Temp. máxima (°C)","Temp. media (°C)",
                        "Temp. mínima (°C)","Precipitación anual (mm)")))

  p11 <- ggplot(comp, aes(anio, value, colour = fuente, shape = fuente)) +
    geom_line(linewidth = 0.5) +
    geom_point(size = 2) +
    scale_colour_manual(values = c("CONAGUA" = "#1b7837",
                                   "NASA POWER" = "#762a83"),
                        name = NULL) +
    scale_shape_manual(values = c("CONAGUA" = 16, "NASA POWER" = 17),
                       name = NULL) +
    scale_x_continuous(breaks = da$anio) +
...
write_summary <- function(data, stats) {
  ...
  w("Fuente clima: NASA POWER (lat ", CONFIG$lat,
    ", lon ", CONFIG$lon, ")")
